<template>
    <div class="__main pb-5">
        <div class="__title mt-3 bg-white px-4 d-flex align-items-center border-bottom" style="height: 50px;">
            <span class="fs-14 fw-semibold text-grey">Sửa Sản Phẩm</span>
        </div>
        <div v-if="loading" class="d-flex align-items-center justify-content-center mt-3" style="min-height: 60vh;">
            <loading__dot />
        </div>
        <div v-else class="__content d-flex align-items-center w-100">
            <div class="__product-add w-100">
                <div class="__basic-info px-4 py-3 bg-white shadow-sm">
                    ...
                    <div class="__content mt-4">
                        <div class="__product-name">
                            ...
                            <input type="text" name="" id="" placeholder="Ex: Áo thun elmall"
                                class="w-100 mt-1 py-1 px-3 fs-14 border" style="height: 40px; border-radius: 3px;"
                        ...
                        <div class="__product-description mt-3">
                            ...
                            <div class="ai-generate-description d-flex align-items-center gap-2 mb-2">
                                <button class="main-btn fs-14 px-3 py-1"
                                    @click="generate_description(product.name)">Sinh mô tả chuẩn Seo</button>
                        ...
                        <div class="__product-images mt-3">
                            ...
                            <div class="__content bg-grey p-3 d-flex gap-2 flex-wrap">
                                <!-- Hình ảnh đã chọn -->
                                <div class="__images position-relative me-2" style="width: 85px; height: 85px;"
                                    v-for="(img, i) in preview_images" :key="i">
                                    <img :src="img" alt="Product Image" class="img-fluid rounded-1"
                                        style="width: 100%; height: 100%; object-fit: cover;">
                                    <div @click="remove_image(i)"
                                        class="remove-img position-absolute cursor-pointer bg-white"
                                        style="z-index: 999; right: -4px; top: -6px; border-radius: 50%; width: 15px; height: 15px;">
                                ...
                                <!-- Nút thêm hình -->
                                <label
                                    class="__add-image text-center border border-dashed rounded p-3 text-danger d-flex flex-column align-items-center justify-content-center"
                                    ...
                                    <div class="mt-1 small fw-semibold fs-12 text-nowrap">Thêm hình <br> ảnh ({{
                                        preview_images.length }}/8)</div>
                                </label>
                        ...
                        <div class="__product-video mt-3">
                            ...
                            <div class="__content bg-grey p-3">
                                <div class="d-flex gap-2 flex-wrap">
                                    <!-- Video đã chọn -->
                                    <div v-if="preview_video" class="position-relative me-2"
                                    ...
                                    <label
                                        class="text-center border border-dashed rounded p-3 text-primary d-flex flex-column align-items-center justify-content-center"
                                        style="width: 85px; height: 85px; cursor: pointer;">
                                    ...
                                    <div class="__notice fs-14 text-muted small ms-3 mt-2">
                                        ...
                                        • Độ dài: 10s-60s<br>
                                        • Định dạng: MP4<br>
                                        • Lưu ý: Sản phẩm có thể hiển thị trong khi video đang được xử lý. Video sẽ tự
                                        động hiển thị sau
                ...
                <div class="__sale-info px-4 bg-white mt-3 shadow-sm">
                    ...
                    <div class="product-classification mt-16">
                        <div class="product-classification__title fs-16 fw-semibold"><span class="text-danger">*</span>
                            Phân loại hàng</div>

                        <div class="product-classification__content bg-grey mt-2" v-if="classification.length"
                            v-for="(group, i) in classification" :key="i">
                                ...
                                <div class="classification box-input-select">
                                    <div class="d-flex align-items-center gap-2">
                                        <input class="input-select" type="text" placeholder="Type or Select"
                                            :value="group.attribute_name" readonly disabled />
                                    </div>
                            ...
                            <div class="d-flex px-4 py-3">
                                ...
                                <div class="classification d-flex flex-wrap gap-2">
                                    <div class="box-input-select box-input-select__item me-4"
                                        v-for="(value, valIndex) in group.attribute_value_objects" :key="valIndex">
                                        <div class="d-flex align-items-center gap-2">
                                            <input class="input-select" type="text" placeholder="Nhập"
                                                :value="value.value" readonly disabled />
                    ...
                    <div class="__classification-list mt-16">
                        ...
                        <div class="__shortcut d-flex fs-14 gap-3 mb-32">
                            <div class="__left d-flex">
                                <span class="border px-1 d-flex align-items-center"
                                    style="border-radius: 3px 0 0 3px; height: 30px;">đ</span>
                                <input type="text" name="" id="" placeholder="Giá" class="outline-none border px-1"
                                    style="border-left: none !important; height: 30px;" v-model="apply_data.price">
                                <input type="text" name="" id="" placeholder="Kho hàng" class="outline-none border px-1"
                                    style="border-left: none !important; height: 30px;" v-model="apply_data.stock">
                                <input type="text" name="" id="" placeholder="SKU phân loại"
                                    class="outline-none border px-1"
                                    style="border-left: none !important; border-radius: 0 3px 3px 0; height: 30px;"
                                    v-model="apply_data.sku">
                            </div>

                            <div class="__right w-100" @click="apply_all">
                                <button class="main-btn w-100 fs-14"
                                    style="height: 30px; border-radius: 2px; min-width: 400px;">Áp
                                    dụng cho tất cả biến thể</button>
                            </div>
                        </div>
                                ...
                                <table class="table fs-14">
                                    <thead class="table-light" style="height: 60px;">
                                        <tr class="fw-500 align-middle text-center">
                                            ...
                                            <td scope="col" class="border-end" v-for="(attr, index) in classification"
                                                :key="index">
                                                {{ attr.attribute_name }}
                                            </td>
                                    ...
                                    <tbody style="height: 100px;">
                                        <tr class="text-center align-middle" v-for="(variant, index) in variants"
                                            :key="index">
                                            <!-- Ảnh biến thể -->
                                            <td class="border-end" style="min-width: 130px;">
                                                <img v-if="variant.preview" :src="variant.preview"
                                                ...
                                                <div class="__img-add p-2"
                                                    ...
                                                    v-else @click="open_modal_variant_image(index)">
                                                    <i class="fa-solid fa-image"
                                                        style="font-size: 30px; color: #888;"></i>
                                                </div>
                                            </td>

                                            <!-- Các giá trị phân loại -->
                                            <td class="border-end fw-semibold"
                                                v-for="(attr, attr_index) in variant.attributes" :key="attr_index"
                                            ...
                                            <td class="border-end" style="min-width: 250px;">
                                                <div class="d-flex justify-content-center">
                                                    <span class="border px-2 d-flex align-items-center fw-500"
                                                        style="border-radius: 3px 0 0 3px; height: 30px;">đ</span>
                                                    <input type="text" v-model="variant.price" placeholder="Nhập giá"
                                            ...
                                            <!-- Nhập kho -->
                                            <td class="border-end" style="min-width: 250px;">
                                                <input type="text" v-model="variant.stock" placeholder="Nhập kho hàng"
                                                    class="outline-none border px-1 fw-500"
                ...
                <div class="__transport px-4 bg-white mt-3 shadow-sm">
                    ...
                    <div class="__content mt-4">
                        <div class="__weight">
                            <div class="__title fs-14"><span class="text-danger">*</span>Cân nặng (sau khi đóng gói)
                            </div>
                            <input type="text" name="" id="" placeholder="Gram"
                                class="w-100 mt-1 py-1 px-3 fs-14 border" style="height: 40px; border-radius: 3px;"
                        ...
                        <div class="__size mt-16 pb-16">
                            ...
                            <div class="d-flex gap-3">
                                <input type="text" name="" id="" placeholder="Dài(cm)"
                                    class="w-100 mt-1 py-1 px-3 fs-14 border" style="height: 40px; border-radius: 3px;"
                                    v-model="product.length">
                                <input type="text" name="" id="" placeholder="Rộng(cm)"
                                    class="w-100 mt-1 py-1 px-3 fs-14 border" style="height: 40px; border-radius: 3px;"
                                    v-model="product.width">
                                <input type="text" name="" id="" placeholder="Cao(cm)"
                                    class="w-100 mt-1 py-1 px-3 fs-14 border" style="height: 40px; border-radius: 3px;"
                ...
                <div class="__submit px-4 py-3 bg-white my-3 shadow-sm d-flex justify-content-end">
                    <button class="border fs-14 fw-500 py-2 px-3 me-2" @click="goBack">Huỷ</button>
                    <button class="main-btn fs-14 fw-500 p-2" @click="submit_product">Cập nhật sản phẩm</button>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="main__content bg-white mt-4 d-flex overflow-x-auto">
                    <div class="__wrapper d-flex">
                        <div class="content__categories d-flex flex-column col-3 pt-3 px-3 overflow-y-auto border-end"
                            ...
                            <div class="category__item d-flex justify-content-between py-2 align-items-center cursor-pointer"
                                :class="{ 'active': selected_level_1_id === c.id }" v-for="c in categories_level_1"
                                :key="c.id" @click="on_select_level_1(c.id)">
                                {{ c.name }}
                                <i class="fa-solid fa-caret-right"></i>
                            </div>
                        ...
                        <div class="content__categories d-flex flex-column col-3 pt-3 px-3 overflow-y-auto border-end"
                            ...
                            <div class="category__item d-flex justify-content-between py-2 align-items-center cursor-pointer"
                                :class="{ 'active': selected_level_2_id === c.id }" v-for="c in categories_level_2"
                                :key="c.id" @click="on_select_level_2(c.id)">
                                {{ c.name }}
                                <i class="fa-solid fa-caret-right"></i>
                            </div>
                        ...
                        <div class="content__categories d-flex flex-column col-3 pt-3 px-3 overflow-y-auto border-end"
                            ...
                            <div class="category__item d-flex justify-content-between py-2 align-items-center cursor-pointer"
                                :class="{ 'active': selected_level_3_id === c.id }" v-for="c in categories_level_3"
                                :key="c.id" @click="on_select_level_3(c.id)">
                                {{ c.name }}
                                <i class="fa-solid fa-caret-right"></i>
                            </div>
                        ...
                        <div class="content__categories d-flex flex-column col-3 pt-3 px-3 overflow-y-auto border-end"
                            ...
                            <div class="category__item d-flex justify-content-between py-2 align-items-center cursor-pointer"
                                :class="{ 'active': selected_level_4_id === c.id }" v-for="c in categories_level_4"
                                :key="c.id" @click="on_select_level_4(c.id)">
                                {{ c.name }}
                                <i class="fa-solid fa-caret-right"></i>
                            </div>
...
const product_id = route.params.id // Lấy id từ URL
...
// Thay đổi: Khi mounted, load dữ liệu sản phẩm
onMounted(async () => {
    try {
        loading.value = true

        // Lấy thuộc tính, shop_id như cũ
        ...
        selected_category.value = prod.category_id

        // Ảnh chính
        preview_images.value = prod.medias.filter(m => m.type == 'image').map(m => `/imgs/products/${m.url}`)
        file_images.value = []

        // Video
        const video_media = prod.medias.find(m => m.type === 'video');
        file_video.value = null
        preview_video.value = video_media ? `/imgs/products/${video_media.url}` : null;

        // Biến thể
        variants.value = (prod.variants || []).map(v => ({
            attributes: (v.variant_attribute || []).map(va => ({
                attribute_id: va.attribute_value.attribute_id,
                attribute_name: attributes.value.find(a => a.id === va.attribute_value.attribute_id)?.name || '',
                value_id: va.attribute_value.id,
                value_name: va.attribute_value.value
            })),
            preview: v.image
                ? (typeof v.image === 'string'
                    ? `/imgs/products/${v.image}`
                    : `/imgs/products/${v.image.url || v.image.path}`)
                : '',
            image: null, // sẽ cập nhật sau khi chọn ảnh
            price: v.price,
            stock: v.stock,
            sku: v.sku ?? ''
        }))
        ...
        if (prod.variants && prod.variants.length > 0) {
            // Gom tất cả các thuộc tính và giá trị từ mọi biến thể
            const attrMap = {};
            prod.variants.forEach(variant => {
                (variant.variant_attribute || []).forEach(va => {
                    const attr_id = va.attribute_value.attribute_id;
                    const attr = attributes.value.find(a => a.id === attr_id);
                    if (!attr) return;
                    if (!attrMap[attr_id]) {
                        attrMap[attr_id] = {
                    ...
                    if (!attrMap[attr_id].attribute_value_objects.some(v => v.id === va.attribute_value.id)) {
                        attrMap[attr_id].attribute_value_objects.push({
            ...
            // Thêm ô trống nếu còn giá trị chưa chọn
            Object.values(attrMap).forEach(group => {
                const selected_ids = group.attribute_value_objects.map(v => v.id);
                const all_ids = group.attribute_values.map(v => v.id);
                const remaining = all_ids.filter(id => !selected_ids.includes(id));
            ...
            classification.value = Object.values(attrMap);
        } else {
        ...
        // Nếu cần load lại category tree, gọi get_categories()
    } catch (error) {
        message.emit("show-message", { title: "Lỗi", text: "Không lấy được dữ liệu sản phẩm!", type: "error" });
...
const submit_product = async () => {
    ...
    // Ảnh chính: chỉ gửi file mới nếu có thay đổi, nếu không gửi tên file cũ
    if (file_images.value.length > 0) {
        file_images.value.forEach((image, index) => {
            formData.append(`file_images[${index}]`, image);
        });
    } else {
        // Gửi danh sách tên file ảnh cũ để backend giữ lại
        preview_images.value.forEach((img, index) => {
            // img có thể là '/imgs/products/abc.jpg' => lấy 'abc.jpg'
            const fileName = img.split('/').pop();
            formData.append(`old_images[${index}]`, fileName);
        });
    }

    // Video: chỉ gửi file mới nếu có thay đổi, nếu không gửi tên file cũ
    if (file_video.value) {
        formData.append('file_video', file_video.value);
    } else if (preview_video.value) {
        // preview_video.value có thể là '/imgs/products/abc.mp4' => lấy 'abc.mp4'
        const fileName = preview_video.value.split('/').pop();
        formData.append('old_video', fileName);
    }
    ...
    // Ảnh riêng từng biến thể: chỉ gửi file mới nếu có thay đổi
    variants.value.forEach((variant, index) => {
        if (variant.image) {
            formData.append(`variant_images[${index}]`, variant.image);
        } else if (variant.preview) {
            // Nếu không đổi, gửi tên file ảnh cũ
            const fileName = variant.preview.split('/').pop();
            formData.append(`old_variant_images[${index}]`, fileName);
        }
    });
    ...
    try {
        ...
        if (res.status === 200) {
            message.emit("show-message", { title: "Thông báo", text: "Cập nhật sản phẩm thành công.", type: "success" });
            router.push('/kenh-nguoi-ban/san-pham');
        }
    } catch (error) {
        message.emit("show-message", { title: "Lỗi", text: "Cập nhật sản phẩm thất bại!", type: "error" });
